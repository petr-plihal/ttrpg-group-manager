...
// For search screen
// Displays all groups based on the search query
...
const GroupsList = ({ loggedId, searchQuery }: { loggedId: number, searchQuery: string }) => {
  ...
  const fetchGroups = async () => {
    try {
      const response = await fetch('http://10.0.2.2:8000/groups/');
      if (!response.ok) {
