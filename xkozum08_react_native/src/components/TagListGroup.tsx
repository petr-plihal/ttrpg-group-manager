...
// show list of tags for groups
// if user is owner of the group , it allows to add new tags with the button next to the header
// also when clicked on the tag, it removes the tag from the user (after Alert confirmation)
// very similar to TagListUser, but for groups
...
const TagListGroup = ({ isOwner, groupId }: { isOwner: boolean , groupId: number}) => {
  ...
  const fetchTags = async () => {
    try {
      const response = await fetch(`http://10.0.2.2:8000/group/${groupId}/tags/`);
      if (!response.ok) {
  ...
  const handleAddTag = async (tagName: string) => {
    ...
    if (tag) {
      assignTag(groupId, tag.pk);
    } else {
      createTag(tagName);
    }
  };

  const assignTag = (groupId: number, tagId: number) => {
    const tagData = {
        group_id: groupId,
        tag_ids: [tagId],
    };
    fetch('http://10.0.2.2:8000/addGroupTags/', {
        method: 'POST',
        headers: {
  ...
  const handleRemoveTag = (tagId: number) => {
    Alert.alert(
      ...
      [
        ...
        { text: 'OK', onPress: () => removeGroupTag(tagId) },
      ],
      { cancelable: false },
    );
  }

  const removeGroupTag = (tagId: number) => {
    const tagData = {
        group_id: groupId,
        tag_ids: [tagId],
    };
    fetch('http://10.0.2.2:8000/removeGroupTags/', {
      method: 'POST',
      headers: {
          'Content-Type': 'application/json',
      },
