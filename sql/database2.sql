...
CREATE TABLE IF NOT EXISTS `ttrpglfg_chat` (
  ...
  `groupid` INT NULL,
  `chattype` ENUM ('APP', 'GRP'),
  PRIMARY KEY (`id`),
  INDEX `fk_application_id_idx` (`applicationid` ASC) VISIBLE,
  INDEX `fk_group_id_idx` (`groupid` ASC) VISIBLE,
  CONSTRAINT `fk_application_chat_ct`
    FOREIGN KEY (`applicationid`)
    REFERENCES `application` (`id`)
    ON DELETE CASCADE
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_group_chat_ct`
    FOREIGN KEY (`groupid`)
    REFERENCES `group` (`id`)
    ON DELETE CASCADE
    ON UPDATE NO ACTION
...
CREATE TABLE IF NOT EXISTS `ttrpglfg_chatmessage` (
  ...
  `chatmessagetype` ENUM ('text', 'sched', 'prefs'),
  `content` VARCHAR(600),
  PRIMARY KEY (`id`),
  INDEX `fk_chat_id_idx` (`chatid` ASC) VISIBLE,
  INDEX `fk_user_id_idx` (`userid` ASC) VISIBLE,
